...
public class PlayerClimb3 : MyMonoInstance<PlayerClimb3>
{
    ...
    private void SetBodyPositionToWall()
    {
        ...
        if ( Mathf.Abs(climbFootPos.position.z - footRayPosition.z) > 0.1f)
        {
            ...
            // 计算旋转轴
            Vector3 rotationAxis = Vector3.Cross(footRayPosition - headRayPosition, Vector3.up).normalized;
    ...
    private void SetClimbInfo(bool qx = false)
    {
        if (isClimb || qx)
        {
